...
for i=1:4
    [f c V]=func_max_imag(MatCorrelacion);%esta funcion me da fila,columna del maximo y su valor
    
    VectorMaximos(i,1 )=f;
    VectorMaximos(i,2 )=c;
    MatCorrelacion((f-tamanio):(f+tamanio),(c-tamanio):(c+tamanio))=0;%tapo en 0 el area
    figure;
    imshow(uint8(MatCorrelacion/max(max(abs(MatCorrelacion)))*255));
end
