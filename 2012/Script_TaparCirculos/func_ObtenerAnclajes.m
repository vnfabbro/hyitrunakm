function [ VectorMaximos ] = func_ObtenerAnclajes(Img)
...
for i=1:4
    [f c V]=func_max_imag(Img);%esta funcion me da fila,columna del maximo y su valor
    VectorMaximos(i,1 )=f;
    VectorMaximos(i,2 )=c;
    Img((f-tamanio):(f+tamanio),(c-tamanio):(c+tamanio))=0;%tapo en 0 el area
    figure;
    imshow(uint8(Img/max(max(abs(Img)))*255));
end
